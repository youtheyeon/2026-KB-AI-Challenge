...
import { createExecution } from '@/entities/verify';
...
const NeedsSimulationView = ({ onPrev }: { onPrev: () => void }) => (
  <div className="space-y-6">
    <h2 className="text-xl font-semibold">실제로 대출금을 어떻게 사용했나요?</h2>
    <div className="space-y-3 rounded border border-border px-5 py-14 text-center">
      <p className="text-sm text-muted-foreground">먼저 검증할 시뮬레이션을 불러와주세요.</p>
...
export const RecordExecutionStep = ({
  ...
  }, [simulationId, retryToken]);

  if (simulationId === null) return <NeedsSimulationView onPrev={onPrev} />;
