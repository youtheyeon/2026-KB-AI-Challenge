...
- [x] **Step 5: 커밋.** `feat: 사업 스냅샷과 근거 기반 진단 모델 추가`.
...
- [x] **Step 1: 실패 테스트 작성.** 대출 입력 계약, 카테고리 최소 5%, 비율 100%, 금액 합계, 재무 결과, 출처 버전을 검증한다.
- [x] **Step 2: 실패 확인.** 타깃 테스트가 변경된 시나리오 API 부재로 실패해야 한다.
- [x] **Step 3: 최소 구현.** 예측 기간과 예측 결과를 제거하고 고정 시나리오와 조건 계산 결과를 저장한다.
- [x] **Step 4: 통과 확인.** 타깃 테스트를 통과시킨다.
- [x] **Step 5: 커밋.** `feat: 고정 시나리오와 재무 비교 모델 반영`.
...
- [x] **Step 1: 실패 테스트 작성.** 새 집행 유형, 실제 배분 합계, 관측값 명칭, 결과 상태, 해결·잔존·신규 병목을 검증한다.
- [x] **Step 2: 실패 확인.** 타깃 테스트가 구형 집행·예측 비교 타입 때문에 실패해야 한다.
- [x] **Step 3: 최소 구현.** 예측 대비 실제 필드를 목표 조건 대비 관측 필드로 교체한다.
- [x] **Step 4: 통과 확인.** 타깃 테스트를 통과시킨다.
- [x] **Step 5: 커밋.** `feat: 집행과 관측 결과 모델 개편`.
...
- [x] **Step 1: 실패 통합 테스트 작성.** Flyway 적용 후 Hibernate `validate`, aggregate cascade, Enum 문자열, 버전 보존, 유일 제약을 검증한다.
- [x] **Step 2: 실패 확인.** 기존 V1과 새 매핑 불일치로 애플리케이션 컨텍스트가 실패해야 한다.
- [x] **Step 3: V1 개편.** 아직 배포되지 않은 초기 마이그레이션을 최종 MVP 테이블과 제약조건으로 교체한다.
- [ ] **Step 4: 최종 검증.** `./gradlew test`, `./gradlew bootJar`, `git diff --check develop...HEAD`를 실행한다.
- [ ] **Step 5: 커밋.** `test: 최종 MVP 도메인 매핑 검증`.

### Review Follow-up

- [x] 집행이 같은 시뮬레이션의 선택 기록을 필수 참조하도록 보완한다.
- [x] 진단 현재값과 비교값의 출처를 분리한다.
- [x] 파일 형식, 입력 소속, 대출·배분 범위 검증을 추가한다.
- [x] 배분 컬렉션 불변성과 카테고리 유일 제약을 추가한다.
- [x] 결과 충족 상태와 시나리오 근거·목표·위험 근거를 제한한다.
