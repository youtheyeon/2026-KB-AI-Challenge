...
- 2026-07-30. 대출 조건은 최종 요청 계약의 금액, 연 금리, 기간, 거치기간, 상환 방식만 저장한다.
- 2026-07-30. 시나리오는 네 배분 카테고리 완전성, 각 5% 이상, 비율 100%, 금액 합계 대출금 일치를 생성 시 검증한다.
- 2026-07-30. 재무 결과는 예측 수치가 아니라 상환액, 반복비용, 현재 상태 유지 시 현금, 손익분기 조건과 규칙 기반 위험을 저장한다.
- 2026-07-30. 시뮬레이션은 데이터셋·기준 스냅샷과 배분 생성기·계산기·프롬프트 버전 및 공공데이터 기준일을 저장한다.
- 2026-07-30. `FinalMvpSimulationDomainTest`와 기존 시뮬레이션 도메인 테스트가 성공했다.
- 2026-07-30. 집행은 선택 기록과 분리하고 `EXACT_SELECTED`, `MODIFIED`, `MIXED`, `CUSTOM`, `MOCK` 유형을 저장한다.
- 2026-07-30. 집행 배분은 카테고리별 실제 금액을 저장하며 합계와 미사용 금액이 대출금액과 일치해야 한다.
- 2026-07-30. 결과 비교 행은 예측값 대신 목표 조건, 관측값, 증감, 충족 상태와 외부 요인을 저장한다.
- 2026-07-30. 재평가는 최신 사업 스냅샷과 해결·잔존·신규 병목 유형을 별도 엔티티로 보존한다.
- 2026-07-30. `FinalMvpOutcomeDomainTest`와 기존 결과 도메인 테스트가 성공했다.
